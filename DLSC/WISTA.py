...
class WISTA:
    ...
    Xorig, X, D = Function.GeneData(X0, X1, info)
    
    record.DT = D.T
    record.Z = np.dot(np.linalg.pinv(D),Xorig)
    
    def forwardP(X, D, info, record):
        n = info.n
        N = info.N
        p = info.p
        I = np.eye(n)
        ...
        Z = C = np.empty((T,n,N))
        t = np.ones((T,n,N))
        H = I - np.dot(DT, D)/alpha + record.dH
        W = DT / alpha + record.dW
        B = np.dot(W,X)
        t[0,:,:] = t[0,:,:]*(lamda/alpha) + record.dt
